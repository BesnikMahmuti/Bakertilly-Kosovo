...
function openClose(drop, btnOpener, holder) {
  $(document).on('click', btnOpener, function (e) {
    if (window.innerWidth < 1024) {
      ...
      } else if($(this).closest(holder).find(drop).length > 0){
        $drop = $(this).closest(holder).find(drop);
        $holder = $(this).closest(holder);
      } else {
        return false;
      }
      ...
      if ($drop.hasClass('expanded')) {
        $drop.slideUp(400).removeClass('expanded');
        $holder.removeClass('expanded');
      } else {
        $(drop).slideUp(400).removeClass('expanded');
        $(holder).removeClass('expanded');
        $drop.slideDown(400).addClass('expanded');
...
function menuDropHover() {
  ...
  if (window.innerWidth > 1024) {
    $('.nav>.main-nav>li').mouseenter(function() {
      $(this).addClass('hover');
      $(this).find('.drop').stop(false, false).slideDown(400);
    });
    $('.nav>.main-nav>li').mouseleave(function() {
      $(this).removeClass('hover');
      $('.drop').stop(false, false).slideUp(400);
    });
  } else{
    $('.has-drop-down-a').on('click', function() {
      event.preventDefault();
      if ($(this).closest('.has-drop-down').hasClass('open-item')) {
        $(this).siblings('.drop').slideUp(300);
        $(this).closest('.has-drop-down').removeClass('open-item');
      } else{
        $('.has-drop-down .drop').slideUp(300);
        $('.has-drop-down').removeClass('open-item');
        $(this).closest('.has-drop-down').addClass('open-item');
...
function menuMobile() {
  var $menuBtn = $('.menu-burger'),
      $menuDrop = $('.mobile-drop-container');

  $menuBtn.on('click', function() {
    if (window.innerWidth < 1024 && !$menuBtn.hasClass('open')) {
      $(this).addClass('open');
      $('body').removeClass('search-open');
      $menuDrop.stop(false, false).slideDown(400);
    } else{
      $menuBtn.removeClass('open');
      $menuDrop.stop(false, false).slideUp(400);
    }
  });

  $(document).mouseup(function (e) {
    if (!$menuDrop.is(e.target) && window.innerWidth < 1024 && $menuDrop.has(e.target).length === 0 && (!$menuBtn.is(e.target))){
      $menuBtn.removeClass('open');
      $menuDrop.stop(false, false).slideUp(400);
    }
  });
...
function initLangDrop() {
  var $languageSwitcher = $('.language-switcher'),
      $languageDrop = $('.language-drop');

  $('.lang-opener').on('click', function() {
    $(this).closest($languageSwitcher).toggleClass('open');
    $languageDrop.slideToggle(400);
  });
  ...
  $('.language-drop li').on('click', function() {
    ...
    $languageDrop.slideUp(400);
    var htmlString = $(this).html();
    $('.lang-selected').html(htmlString);
  });

  $(document).mouseup(function (e) {
    if (!$languageSwitcher.is(e.target)
    && $languageSwitcher.has(e.target).length === 0)
    {
      $languageDrop.slideUp(400);
      $languageSwitcher.removeClass('open');
    }
...
function initSearchContainer() {
  var $btnOpener = $('.search-btn-opener'),
      $serchContainer = $('.search-container');
  ...
  $(document).mouseup(function (e) {
    if (!$serchContainer.is(e.target) && $serchContainer.has(e.target).length === 0 && (!$serchContainer.is(e.target))){
      $('body').removeClass('search-open');
      $('#search').blur();
    }
  });
...
function cookieMsg () {
  ...
  function checkCookie() {
      var cookieMsg = getCookie('cookie_name');
